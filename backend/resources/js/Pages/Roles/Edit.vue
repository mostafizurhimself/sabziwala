<template>
	<form-view @submitted="update('roles.update', role.id)" title="Edit Role" :breadcrumb="breadcrumb">
		<template #form>
			<!-- Name -->
						...
						<div v-for="row in permissions" :key="row.id">
							...
							<div v-for="col in row.permissions" :key="col.id" class="mt-1">
								<label class="flex items-center cursor-pointer text-sm">
									<jet-checkbox class="mr-2" :value="col.id" v-model:checked="form.permissions" @update:change="onSelectCol(row)" />
									<span>{{ col.label }}</span>
								</label>
							</div>
						</div>
		...
		<template #actions>
			...
			<jet-button @click.prevent="update('roles.update', role.id, true)" class="px-6 xs:mr-2 my-2 xs:my-0" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">Update & Continue</jet-button>
